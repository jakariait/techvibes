...
const GeneralProfileInfoSection = () => {
  ...
  const closeSnackbar = () => setSnackbar({ open: false, message: "", type: "success" });
  ...
  return (
    <div className="bg-[#212F35] inner-glow p-4 rounded-xl mb-6 max-w-6xl mx-auto">
      <div className="flex items-center gap-2 mb-4 justify-center">
        ...
        <h2 className="text-base font-medium text-green-400">General Profile Info</h2>
      </div>

      {/* Dynamic text fields */}
      {[...alwaysFields, ...(isCorporate ? corporateFields : normalUserFields)].map((field) => (
        <div key={field} className="flex flex-col md:flex-row md:items-center gap-2 mb-3">
          <label className="text-white md:w-[180px] shrink-0">{properLabel(field)}:</label>
          <input
            type="text"
      ...
      {isCorporate && (
        <div className="flex flex-col md:flex-row md:items-center gap-2 mb-3">
          <label className="text-white md:w-[180px]">Profile Photo Shape:</label>
          <select
            value={fields.profilePhotoShape}
            onChange={(e) => handleFieldChange("profilePhotoShape", e.target.value)}
            className="bg-[#212F35] text-white p-2 rounded border border-gray-600 flex-1 focus:outline-none"
          >
      ...
      <div className="flex flex-col md:flex-row md:items-center gap-2 mb-3">
        ...
        <div className="flex items-center gap-2">
          <input
            ...
            onChange={(e) => handleFieldChange("qrCodeIsActive", e.target.checked)}
          />
          <span className="text-white">{fields.qrCodeIsActive ? "Enabled" : "Disabled"}</span>
        </div>
      </div>
        ...
        <>
          <div className="flex flex-col md:flex-row md:items-center gap-2 mb-3">
            ...
            <input
              ...
              onChange={(e) => handleNestedFieldChange("businessHours", "start", e.target.value)}
            ...
            <input
              ...
              onChange={(e) => handleNestedFieldChange("businessHours", "end", e.target.value)}
              className="bg-[#212F35] text-white p-2 rounded border border-gray-600 focus:outline-none"
            />
          </div>
          ...
          <div className="flex flex-col md:flex-row md:items-center gap-2 mb-3">
            ...
            <input
              ...
              onChange={(e) => handleNestedFieldChange("businessDay", "start", e.target.value)}
              className="bg-[#212F35] text-white p-2 rounded border border-gray-600 focus:outline-none"
            />
            <input
              type="text"
              ...
              onChange={(e) => handleNestedFieldChange("businessDay", "end", e.target.value)}
              className="bg-[#212F35] text-white p-2 rounded border border-gray-600 focus:outline-none"
            />
          </div>

          <div className="flex flex-col md:flex-row md:items-center gap-2 mb-3">
            <label className="text-white md:w-[180px]">Business Time Zone:</label>
            <input
              type="text"
              placeholder="e.g. BST"
              value={fields.businessTimeZone}
              onChange={(e) => handleFieldChange("businessTimeZone", e.target.value)}
              className="bg-[#212F35] text-white p-2 rounded border border-gray-600 flex-1 focus:outline-none"
            />
          </div>
        </>
