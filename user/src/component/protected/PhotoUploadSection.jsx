...
const PhotoUploadSection = ({ type = "profilePhoto" }) => {
  ...
  const handleUpload = async (e) => {
    ...
    formData.append(type, file); // ✅ this must match multer field name
    formData.append("field", type); // optional if backend reads it
    ...
    try {
      ...
      // ✅ update photo if present in response
  ...
  const handleDelete = async () => {
    ...
    const confirmed = window.confirm(`Remove current ${label}?`);
    if (!confirmed) return;
